...
A **dbt** project modeling the synthetic **RavenStack SaaS** dataset (by River @ Rivalytics)
to analyze subscription churn, retention, feature adoption, support load, and revenue.

- **Warehouse:** Google BigQuery — raw data lives in the `subscriptionchurn.raw` dataset.
- **Layers:** `staging` (views) → `intermediate` (ephemeral) → `warehouse` (tables).

## Setup

```bash
pip install dbt-bigquery   # BigQuery adapter
dbt deps                   # install packages (e.g. dbt_utils) from packages.yml
dbt build                  # run + test + seed + snapshot
```

The profile (`subscription_churn`) is **not** in the repo — it lives in `~/.dbt/profiles.yml`
and must target the `subscriptionchurn` BigQuery project.
...
_Dataset: **RavenStack** synthetic SaaS data — credit River @ Rivalytics (MIT-like, fully synthetic, no PII)._